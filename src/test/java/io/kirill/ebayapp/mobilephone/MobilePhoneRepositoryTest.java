...
class MobilePhoneRepositoryTest {
  ...
  @Test
  void existsByUrl() {
    var exists = Flux.fromStream(IntStream.range(1, 11).boxed())
        .map(i -> MobilePhoneBuilder.iphone6s().model("iphone " + i).url("ebay.com").build())
        .flatMap(template::save)
        .then(mobilePhoneRepository.existsByUrl("ebay.com"));
    ...
    StepVerifier
        .create(mobilePhoneRepository.existsByUrl("google.com"))
        .expectNextMatches(e -> !e)
        .verifyComplete();
  }
